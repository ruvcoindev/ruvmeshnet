...
Note that Ruvmeshnet will automatically populate sane defaults for any
...
// options that are necessary for an Ruvmeshnet node to run. You will need to
// supply one of these structs to the Ruvmeshnet core when starting a node.
type NodeConfig struct {
	...
	AdminListen         string                     `json:",omitempty" comment:"Listen address for admin connections. Default is to listen for local\nconnections either on TCP/9090 or a UNIX socket depending on your\nplatform. Use this value for ruvmeshnetctl -endpoint=X. To disable\nthe admin socket, use the value \"none\" instead."`
	...
	NodeInfoPrivacy     bool                       `comment:"By default, nodeinfo contains some defaults including the platform,\narchitecture and Ruvmeshnet version. These can help when surveying\nthe network and diagnosing network routing problems. Enabling\nnodeinfo privacy prevents this, so that only items specified in\n\"NodeInfo\" are sent back if specified."`

...
// Ruvmeshnet mobile package is meant to "plug the gap" for mobile support, as
...
// Send sends a packet to Ruvmeshnet. It should be a fully formed
// IPv6 packet
...
// Recv waits for and reads a packet coming from Ruvmeshnet. It
...
func (m *Ruvmeshnet) Stop() error {
	logger := log.New(m.log, "", 0)
	logger.EnableLevel("info")
	logger.Infof("Stopping the mobile Ruvmeshnet instance %s", "")
	if m.multicast != nil {
	...
	logger.Infof("Stopping Ruvmeshnet core %s", "")
	m.core.Stop()
	return nil
}
